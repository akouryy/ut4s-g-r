...
export const R2Report: React.FC<NoChild> = React.memo(() => {
  ...
  const exB2 = setter(new R2Algo('Bezier'), [p0, p1, p2, p3, p4])
  const exB3 = setter(new R2Algo('Bezier', { deCasteljau: true }), [p0, p1, p2, p3])
  const exB4 = setter(new R2Algo('Bezier'), [p0, p1w, p2w, p3])
  const exB5 = setter(new R2Algo('Bezier'),
  ...
  return (
    <section className='R2Report'>
      ...
      <p>
        ...
        <LinkButton onClick={exB1}>例</LinkButton>
        {', '}
        <LinkButton onClick={exB2}>3次元空間の例</LinkButton>
        )と de Casteljau のアルゴリズム(
        <LinkButton onClick={exB3}>例</LinkButton>
        )の双方を実装した。さらに、有理ベジェ曲線も単純な多項式評価により実装した(
        <LinkButton onClick={exB4}>例</LinkButton>
        )。これにより円弧(の一部)を原理的には正確に描画できる(
        <LinkButton onClick={exB5}>例</LinkButton>
        {', '}
      ...
      <p>
        ...
        を参考に κ-Curves を実装した(
        <LinkButton onClick={exK1}>例</LinkButton>
